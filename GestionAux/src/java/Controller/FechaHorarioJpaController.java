    ...
    public void create(FechaHorario fechaHorario) throws PreexistingEntityException, Exception {
        ...
        try {
            ...
            for (Hora horaListHoraToAttach : fechaHorario.getHoraList()) {
                horaListHoraToAttach = em.getReference(horaListHoraToAttach.getClass(), horaListHoraToAttach.getHoraId());
            ...
            for (Hora horaListHora : fechaHorario.getHoraList()) {
                FechaHorario oldIdFechaHorarioOfHoraListHora = horaListHora.getIdFechaHorario();
                horaListHora.setIdFechaHorario(fechaHorario);
                horaListHora = em.merge(horaListHora);
                if (oldIdFechaHorarioOfHoraListHora != null) {
                    oldIdFechaHorarioOfHoraListHora.getHoraList().remove(horaListHora);
                    oldIdFechaHorarioOfHoraListHora = em.merge(oldIdFechaHorarioOfHoraListHora);
                }
            }
        ...
        try {
            ...
            for (Hora horaListOldHora : horaListOld) {
                if (!horaListNew.contains(horaListOldHora)) {
                    ...
                    illegalOrphanMessages.add("You must retain Hora " + horaListOldHora + " since its idFechaHorario field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            for (Hora horaListNewHoraToAttach : horaListNew) {
                horaListNewHoraToAttach = em.getReference(horaListNewHoraToAttach.getClass(), horaListNewHoraToAttach.getHoraId());
            ...
            for (Hora horaListNewHora : horaListNew) {
                if (!horaListOld.contains(horaListNewHora)) {
                    FechaHorario oldIdFechaHorarioOfHoraListNewHora = horaListNewHora.getIdFechaHorario();
                    horaListNewHora.setIdFechaHorario(fechaHorario);
                    horaListNewHora = em.merge(horaListNewHora);
                    if (oldIdFechaHorarioOfHoraListNewHora != null && !oldIdFechaHorarioOfHoraListNewHora.equals(fechaHorario)) {
                        oldIdFechaHorarioOfHoraListNewHora.getHoraList().remove(horaListNewHora);
                        oldIdFechaHorarioOfHoraListNewHora = em.merge(oldIdFechaHorarioOfHoraListNewHora);
                    }
                }
            }
            em.getTransaction().commit();
    ...
    public void destroy(String id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            for (Hora horaListOrphanCheckHora : horaListOrphanCheck) {
                ...
                illegalOrphanMessages.add("This FechaHorario (" + fechaHorario + ") cannot be destroyed since the Hora " + horaListOrphanCheckHora + " in its horaList field has a non-nullable idFechaHorario field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
